<form method="post" enctype="multipart/form-data" action="<?=admin_url('shop', 'product', 'update', ['product' => $product->id])?>" autocomplete="off">
    <?=tw_csrf(true)?>
    <div class="row">
        <div class="col-sm-8">
                ...
                <div class="table-responsive">
                    <table class="table table-striped">
                        <tr>
                            ...
                            <td style="width: 42%">
                                <input class="form-control" type="text" name="name" maxlength="255" value="<?=$product->name?>" required>
                            </td>
                        </tr>
                        <tr>
                            <td>
                            ...
                            <td>
                                <select name="server" class="uniform">
                                    ...
                                    <?php foreach ($servers as $server): ?>
                                        <option value="<?=$server->getId()?>" <?=($server->getId() === $product->server_id ? 'selected' : '')?>><?=$server->name?></option>
                                    <?php endforeach; ?>
                                </select>
                            </td>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td>
                                <input class="form-control" type="text" name="block_id" maxlength="255" value="<?=$product->block_id?>" required>
                            </td>
                        </tr>
                        <tr>
                            <td>
                            ...
                            <td>
                                <input class="form-control" type="number" name="amount" min="1" value="<?=$product->amount?>" required>
                            </td>
                        </tr>
                        <tr>
                            <td>
                        ...
                        <tr>
                            ...
                            <td>
                                <input class="switch" type="checkbox" name="enabled" <?=($product->enabled ? 'checked' : '')?> value="1">
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <h6 class="media-heading text-semibold">Иконка <img src="<?=$product->getImg()?>" alt="" style="width: 32px;height: 32px;"></h6>
                    ...
                    <table class="table table-striped">
                        ...
                        <?php foreach ($enchants['default'] as $enchantId => $enchantName): ?>
                            <tr>
                                ...
                                <td style="width: 32%">
                                    <input class="form-control" type="number" name="enchants[<?=$enchantId?>]" min="0" max="100" value="<?=isset($product->enchants[$enchantId]) ? $product->enchants[$enchantId]->getLevel() : 0?>" required>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                        </tbody>
...
<script type="text/javascript">
    ...
    const productEnchants = <?=json_encode($product->getAttributes()['enchants'])?>

    function setServerEnchants(server) {
        if (!server || !enchants[server]) {
            ...
            {
                const level = productEnchants[id] ? productEnchants[id] : 0
                out += '<tr><td><h6 class="media-heading text-semibold">' + enchants[server][id] + '</h6></td><td><input class="form-control" type="number" name="enchants[' + id + ']" min="0" max="100" value="' + level + '" required></td></tr>'
            }
    ...
    $(document).ready(() => {
        setServerEnchants('<?=$product->server_id?>')
    })

    $('select[name="server"]').on('change', function (event) {
        setServerEnchants(event.target.value)
    })
</script>
