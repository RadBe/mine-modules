...
final class AttributeException extends NestedValidationException implements NonOmissibleException
{
    ...
    protected $defaultTemplates = [
        self::MODE_DEFAULT => [
            self::NOT_PRESENT => 'Атрибут {{name}} должен присутствовать',
            self::INVALID => 'Атрибут {{name}} должен быть действительным',
        ],
        self::MODE_NEGATIVE => [
            self::NOT_PRESENT => 'Атрибут {{name}} не должен присутствовать',
            self::INVALID => 'Атрибут {{name}} не должен проверяться',
        ],
    ];
